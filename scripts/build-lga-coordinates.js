// scripts/build-lga-coordinates.js
const fs = require("fs");
const path = require("path");
const fetch = require("node-fetch");

const CACHE_FILE = path.join(__dirname, "../lga-coordinates-cache.json");
const OUTPUT_FILE = path.join(__dirname, "../lga-coordinates.json");
...
async function geocode(lga, district) {
  ...
  for (let q of queries) {
    ...
    try {
      const res = await fetch(url, {
        headers: {
          "User-Agent": "AlertZone-LGA-Coordinates-Builder-V2 (contact: thamaruj@gmail.com)"
        }
      });
      ...
      try {
        ...
        console.warn(`⚠️ Warning: Nominatim returned non-JSON response for "${q}". Content snippet: ${text.slice(0, 100)}`);
        // Do not cache fallbacks on network block/errors
      ...
      if (data && data.length > 0) {
        ...
        console.log(`✅ Geocoded: ${cacheKey} -> ${coords.lat}, ${coords.lng} (using query: "${q}")`);
        return coords;
      }
    } catch (err) {
  ...
  console.log(`⚠️ Failed geocoding for "${cacheKey}". Falling back to district center.`);
  ...
  // Only cache if not a blocked page
...
async function run() {
  ...
  // Active districts only to minimize Nominatim calls
  const activeDistricts = ["Colombo", "Gampaha", "Kalutara", "Ratnapura", "Kegalle", "Anuradhapura"];
  
  // Flatten all LGAs to geocode
  const queue = [];
  for (const [prov, dists] of Object.entries(sriLankaGeographics)) {
    for (const [dist, lgas] of Object.entries(dists)) {
      if (activeDistricts.includes(dist)) {
  ...
  console.log(`🚀 Starting geocoding of ${queue.length} LGAs in active districts...`);
  
  for (let i = 0; i < queue.length; i++) {
    const item = queue[i];
    console.log(`[${i + 1}/${queue.length}] Geocoding: ${item.dist} -> ${item.lga}`);
    ...
    // Add safety delay after each LGA
    await new Promise(r => setTimeout(r, 1000));
  }

  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(result, null, 2), "utf8");
  console.log(`🎉 Completed! Output saved to: ${OUTPUT_FILE}`);
}
